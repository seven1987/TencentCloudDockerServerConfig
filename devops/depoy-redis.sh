...
#(cd ${WORKSPACE}/devops/;chmod +x *.sh; ./deplay-redis.sh)

SERVICE_NAME="redis"
RELATIVE_PATH="/dnf/redis"
...
if [ $rebuild == true ]; then
    ...
    (ssh ${REMOTE_HOST} "cd ${REMOTE_PATH};
        ...
        if [ ! -d "$SERVICE_NAME" ]; then
            ...
            (cd ${SERVICE_NAME}${RELATIVE_PATH}; mkdir redis_data; sudo docker-compose down; sudo docker-compose up -d --build)
        else
	...
	(ssh ${REMOTE_HOST} "cd ${REMOTE_PATH};
      (cd ${SERVICE_NAME}${RELATIVE_PATH}; ls)
            (sudo docker restart dnf_redis_1);
     ")
fi